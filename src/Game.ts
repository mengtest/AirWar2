...
class Game {
    ...
    private hero: Role;
    //子弹发射偏移位置表
    private bulletPos: Array<any> = [[-15, 15], [-30, 0, 30], [-45, -15, 15, 45]];
        ...
        for (var i: number = Laya.stage.numChildren - 1; i > 0; i--) {
            ...
            if (role.shootType > 0) {
                ...
                if (time > role.shootTime) {
                    ...
                    // //从对象池里面创建一个子弹
                    // var bullet1: Role = Laya.Pool.getItemByClass("role", Role);
                    // //初始化子弹信息
                    // bullet1.init("bullet1", role.camp, 1, -10, 1);
                    // //设置角色类型为子弹类型
                    // bullet1.isBullet = true;
                    // //设置子弹发射初始化位置
                    // bullet1.pos(role.x, role.y - role.hitRadius - 10);
                    //添加到舞台上
